...
analysis.fml<-as.formula(completion~par+child_gender+gender+yr_exp+
                           hospstatus+kemri_cme+parXkemri_cme+(1|hosp:clinician)+
                           (1|hosp) )
...
fit.m<-with.mitml.list(impList, lmer(as.formula(completion~par+child_gender+gender+yr_exp+
                                                  hospstatus+kemri_cme+par:kemri_cme+(1|hosp:clus)+
                                                  (1|hosp) )))
...
clus<-kemri1$clinician
setwd("C:/Users/rmjlmqu/Documents")
impsb<-jomo(Y=Y, Y2=Y2, X=X, X2=X2, clus=clus, nburn = 12000, nbetween=2000, nimp=10, meth="random") 
...
X2<-X
clus<-kemri0$clinician
impsb2<-jomo(Y=Y, Y2=Y2, X=X, X2=X2, clus=clus, nburn = 12000, nbetween=2000, nimp=10, meth="random")
...
X2<-X
clus<-kemri1$clinician
impsb3<-jomo(Y=Y, Y2=Y2, X=X, X2=X2, clus=clus, nburn = 12000, nbetween=2000, nimp=10, meth = "random")
...
X2<-X
clus<-kemri0$clinician
impsb4<-jomo(Y=Y, Y2=Y2, X=X, X2=X2, clus=clus, nburn = 12000, nbetween=2000, nimp=10, meth = "random")
...
impsb3$par<-1
impsb4$par<-0
impsb$hosp<-ifelse(impsb$hosp6==1,6,ifelse(impsb$hosp7==1,7,
                                           ifelse(impsb$hosp8==1,8,5)))
impsb2$hosp<-ifelse(impsb2$hosp6==1,6,ifelse(impsb2$hosp7==1,7,
                                             ifelse(impsb2$hosp8==1,8,5)))
impsb3$hosp<-ifelse(impsb3$hosp2==1,2,ifelse(impsb3$hosp3==1,3,
                                             ifelse(impsb3$hosp4==1,4,1)))
impsb4$hosp<-ifelse(impsb4$hosp2==1,2,ifelse(impsb4$hosp3==1,3,
                                             ifelse(impsb4$hosp4==1,4,1)))
impsb<-impsb[,-which(colnames(impsb)%in%c("hosp58","hosp6","hosp7","hosp8"))]
impsb2<-impsb2[,-which(colnames(impsb2)%in%c("hosp58","hosp6","hosp7","hosp8"))]
impsb3<-impsb3[,-which(colnames(impsb3)%in%c("hosp14","hosp2","hosp3","hosp4"))]
impsb4<-impsb4[,-which(colnames(impsb4)%in%c("hosp14","hosp2","hosp3","hosp4"))]

impsb$kemri_cme<-0
impsb2$kemri_cme<-0

imputs2<-data.frame(rbind(impsb,impsb2,impsb3,impsb4))
imputs2<-imputs2[order(imputs2$Imputation),]
imputs2$id<-1:nrow(kemri)

impList2 <- jomo2mitml.list(imputs2)

fit.m2<-with.mitml.list(impList2, lmer(as.formula(completion~par+child_gender+gender+yr_exp+
                                                    hospstatus+kemri_cme+par:kemri_cme+(1|hosp:clus)+
                                                    (1|hosp) )))
...
smcimp.fml<-as.formula(completion~par+child_gender+gender+yr_exp+
...
fit.m3<-with.mitml.list(implist3, lmer(as.formula(completion~par+child_gender+gender+yr_exp+
                                                    hospstatus+kemri_cme+par:kemri_cme+(1|hosp:clus)+
                                                    (1|hosp) )))
...
save(fit.CR, fit.MI, fit.MI.het, fit.MI.SMC, file="Results_Example_rim.RData")
