##############################################
####  Level-2 imp. Cross-level interaction####
##############################################
...
kemri$scaled.age<-scale(kemri$yr_exp)
kemri$par<-factor(kemri$par)
analysis.fml<-as.formula(completion~par+scaled.age+par:scaled.age+(1+par|clinician))
fit.CR<-lmer(analysis.fml, data=kemri )
summary(fit.CR)

Y<-kemri[,c("par"), drop=FALSE]
Y2<-kemri[,c("scaled.age"), drop=FALSE]
clus<-kemri[,c("clinician")]
jointmodel<-jomo.MCMCchain(Y=Y, Y2=Y2, clus=clus, nburn=1000)
...
mu2<-apply(jointmodel$collect.l2.beta, c(1,2),mean)
...
analysis.model<-as.formula(Y~par+scaled.age+par:scaled.age+(1+par|clus))
sensitivity.par<-2
beta.smc[3:4]<-sensitivity.par*beta.smc[3:4]
...
for (i in 1:n.sim) {
  ...
  Xvar<-rnorm(n.obs,mu,sqrt(omega))+v[clus,1]
  
  X<-data.frame(1, as.numeric(Xvar<0), v[clus,2], as.numeric(Xvar<0)*v[clus,2])
  Z<-data.frame(1, as.numeric(Xvar<0))
  
  colnames(X)<-c("cons","par","scaled.age", "inter")
  ...
  for (j in 1:n.clus) {
    if (runif(1,0,1)<p.miss.age.MAR[j]) data.miss[clus==j,"scaled.age"]<-NA
  }
  ...
  fit.m<-with(implist, lmer(Y~par+scaled.age+par:scaled.age+(1+par|clus)))
  ...
  fit.m2<-with(implist2, lmer(Y~par+scaled.age+par:scaled.age+(1+par|clus)))
  ...
  imps3<-jomo.lmer(data.miss, formula = analysis.model, level = c(1,1,2,1), nburn=n.burn, 
                   nbetween=n.between, nimp=n.imp)
  implist3 <- jomo2mitml.list(imps3)
  
  fit.m3<-with(implist3, lmer(Y~par+scaled.age+par:scaled.age+(1+par|clus)))
...
#save.image( file="Results_Level2_Interaction_sens2.RData")
