##############################################
####  Interaction Scenario.Sensitiviy 2X. ####
##############################################
...
# Sensitivity to larger quadratic effect
sensitivity.par<-2
beta.smc[4]<-sensitivity.par*beta.smc[4]
...
for (i in 1:n.sim) {
  ...
  X<-data.frame(1, as.numeric(Xvar[,2]<0), Xvar[,1], Xvar[,1]*as.numeric(Xvar[,2]<0))
  Z<-data.frame(1, as.numeric(Xvar[,2]<0))
  
  colnames(X)<-c("cons","par","scaled.age","int")
...
#save.image( file="Results_Interaction_sens2.RData")
