##############################################
####  Base-case Scenario. Random-interc.  ####
##############################################
...
analysis.fml<-as.formula(completion~par+scaled.age+(1|clinician))
...
analysis.model<-as.formula(Y~par+scaled.age+(1|clus))
...
for (i in 1:n.sim) {
  ...
  Y<-rnorm(n.obs,as.matrix(X)%*%beta.smc,res.smc)+u[clus]
  ...
  rev.FD[i]<-as.numeric(VarCorr(fit.FD))^2
  ...
  rev.CR[i]<-as.numeric(VarCorr(fit.CR))^2
  ...
  fit.m<-with(implist, lmer(Y~par+scaled.age+(1|clus)))
  ...
  fit.m2<-with(implist2, lmer(Y~par+scaled.age+(1|clus)))
  ...
  fit.m3<-with(implist3, lmer(Y~par+scaled.age+(1|clus)))
...
# save.image( file="Results_Random_Intercept.RData")
