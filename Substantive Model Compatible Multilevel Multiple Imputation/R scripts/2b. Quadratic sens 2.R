##############################################
####  Quadratic Scenario. Senstivity 2X.  ####
##############################################
...
analysis.fml<-as.formula(completion~par+scaled.age+I(scaled.age^2)+(1+par|clinician))
...
analysis.model<-as.formula(Y~par+scaled.age+I(scaled.age^2)+(1+par|clus))
...
for (i in 1:n.sim) {
  ...
  X<-data.frame(1, as.numeric(Xvar[,2]<0), Xvar[,1], Xvar[,1]^2)
  Z<-data.frame(1, as.numeric(Xvar[,2]<0))
  
  colnames(X)<-c("cons","par","scaled.age","s2")
  ...
  fit.m<-with(implist, lmer(Y~par+scaled.age+I(scaled.age^2)+(1+par|clus)))
  ...
  fit.m2<-with(implist2, lmer(Y~par+scaled.age+I(scaled.age^2)+(1+par|clus)))
  ...
  fit.m3<-with(implist3, lmer(Y~par+scaled.age+I(scaled.age^2)+(1+par|clus)))
...
# save.image( file="Results_Quadratic_sens2.RData")
