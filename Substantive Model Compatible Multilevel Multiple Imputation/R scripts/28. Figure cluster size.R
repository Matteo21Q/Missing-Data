...
load("Results_smallclusters 15.RData")
...
load("Results_smallclusters 5.RData")
...
pdf("Small clusters plot.pdf", ,width=14,height=7)
m <- matrix(c(1,2,3,3),nrow = 2,ncol = 2,byrow = TRUE)
layout(mat = m,heights = c(0.85,0.15))

plot(res[res$method=="MI.hom","magnitude"], res[res$method=="MI.hom","estimate"],
     main="PAR fixed-effect estimate", xlab="Average cluster size", ylab="Relative bias (%)",
     xaxt="n", yaxt="n", ylim=c(-150,150), type="b")
polygon(x=c(75,15,5,5,15,75), y= c(res[res$method=="MI.hom","low_est"],
                               rev(res[res$method=="MI.hom","up_est"])), col="grey79", border="NA")
mycol <- rgb(0, 0, 255, max = 255, alpha = 25, names = "blue50")
polygon(x=c(75,15,5,5,15,75), y= c(res[res$method=="MI.het","low_est"],
                               rev(res[res$method=="MI.het","up_est"])), col=mycol, border="NA")
mycol2 <- rgb(0, 255, 0, max = 255, alpha = 25, names = "green50")
polygon(x=c(75,15,5,5,15,75), y= c(res[res$method=="MI.SMC","low_est"],
...
axis(1, at=c(5, 15, 75), labels=c("5","15","75"))
axis(2, at=seq(-150,150,50), labels = paste(seq(-150,150,50),"%", sep=""), las=2)

plot(res[res$method=="MI.hom","magnitude"], res[res$method=="MI.hom","var"],
     main="Random slope variance", xlab="Average cluster size", ylab="Relative bias (%)",
     xaxt="n", yaxt="n", ylim=c(-50,50), type="b")
polygon(x=c(75,15,5,5,15,75), y= c(res[res$method=="MI.hom","low_var"],
                               rev(res[res$method=="MI.hom","up_var"])), col="grey79", border="NA")
mycol <- rgb(0, 0, 255, max = 255, alpha = 25, names = "blue50")
polygon(x=c(75,15,5,5,15,75), y= c(res[res$method=="MI.het","low_var"],
                               rev(res[res$method=="MI.het","up_var"])), col=mycol, border="NA")
mycol2 <- rgb(0, 255, 0, max = 255, alpha = 25, names = "green50")
polygon(x=c(75,15,5,5,15,75), y= c(res[res$method=="MI.SMC","low_var"],
...
lines(res[res$method=="MI.SMC","magnitude"], res[res$method=="MI.SMC","var"],
      type="b", pch=15, lty=3, col="darkgreen")
axis(1, at=c(5,15,75), labels=c("5","15","75"))
axis(2, at=seq(-50,50,25), labels = paste(seq(-50,50,25),"%", sep=""), las=2)
