...
analysis.fml<-as.formula(par~child_gender+scaled.age+(1|clinician))
fit.CR<-glmer(analysis.fml, data=kemri, family = binomial )
summary(fit.CR)
...
reff.smc<-as.numeric(VarCorr(fit.CR))
...
for (i in 1:n.sim) {
  ...
  colnames(X)<-c("cons","child_gender","scaled.age")
  
  u<-rnorm(n.clus,0,sqrt(reff.smc))
  pY<-inv.logit(as.matrix(X)%*%beta.smc+u[clus])
  Y<-rbinom(n.obs,1,pY)
  ...
  fit.FD<-glmer(analysis.model, data = data.sim, family=binomial)
  ...
  p.miss.cg.MAR<-(1+exp(2-1.5*data.miss$Y))^(-1)
  ...
  fit.CR<-glmer(analysis.model, data = data.miss, family=binomial)
  ...
  fit.m<-with(implist, glmer(Y~child_gender+scaled.age+(1|clus), family=binomial))
  ...
  rev.MI.hom[i]<- fit.JM.hom$var.comp[1]
  ...
  fit.m2<-with(implist2, glmer(Y~child_gender+scaled.age+(1|clus), family=binomial))
  ...
  rev.MI.het[i]<- fit.JM.het$var.comp[1]
  
  # SMC JM MI
  
  imps3<-jomo.glmer(data.miss, formula = analysis.model, nburn=n.burn,
                    nbetween=n.between, nimp=n.imp, family="binomial")
  implist3 <- jomo2mitml.list(imps3)
  
  fit.m3<-with(implist3, glmer(Y~child_gender+scaled.age+(1|clus), family=binomial))
  ...
  rev.MI.SMC[i]<- fit.JM.SMC$var.comp[1]
...
# save.image( file="Results_GLMER.RData")
