##############################################################
####  Generate table of results. Few clusters and syst    ####
##############################################################
...
##############################################
#### Load results few clusters 6          ####

load("Results_fewclusters6.RData")
...
##############################################
#### Load results sensitivity 12           ####

load("Results_fewclusters12.RData")
...
##############################################
#### Load results Systematically          ####

load("Results_Systematically.RData")

# Set true values:
Table.of.results[13,c(2,7,12)]<-beta.smc
...
for ( i in 1: length(meths)) {
  ...
  Table.of.results[13+i,c(2,7,12)]<-apply(get(paste("est",meths[i],sep=".")),2,mean)
  Table.of.results[13+i,c(3,8,13)]<-abs((Table.of.results[13+i,c(2,7,12)]-beta.smc)/beta.smc)*100
  Table.of.results[13+i,c(4,9,14)]<-apply(get(paste("se",meths[i],sep=".")),2,mean)
  Table.of.results[13+i,c(5,10,15)]<-apply(get(paste("est",meths[i],sep=".")),2,sd)
  Table.of.results[13+i,c(6,11,16)]<-apply(get(paste("cov",meths[i],sep=".")),2,mean)*100
...
write.table(Table.of.results, "Table_h.txt", sep=",")